...
# echo "$($PSQL "SELECT UNNEST(array[winners.name, opponents.name]) AS name FROM games LEFT JOIN teams AS winners ON games.winner_id = winners.team_id LEFT JOIN teams AS opponents ON games.opponent_id = opponents.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"
